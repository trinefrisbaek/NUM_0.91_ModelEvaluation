...
rand_kc_min=2.83523449187784e-05; rand_kc_max=0.0599994211312315;
...
min_params=[r_star_d_min,rand_y_min,rand_rlstar_min,aF_random_min,c_passive_random_min,alphaMax_rand_min,alphaR_rand_min,mortHTL_rand_min,rand_rhocn_min,rand_fracHTLN_min,rand_epsilonL_min,...
    rand_alphaN_min,rand_epsilonF_min,rand_cF_min,rand_beta_min,rand_sigma_min,rand_remin2_min,rand_reminF_min,rand_rhoval_min,rand_mort2val_min,rand_remPOM_min,rand_vel1_min,rand_vel2_min,rand_kc_min];
max_params=[r_star_d_max,rand_y_max,rand_rlstar_max,aF_random_max,c_passive_random_max,alphaMax_rand_max,alphaR_rand_max,mortHTL_rand_max,rand_rhocn_max,rand_fracHTLN_max,rand_epsilonL_max,...
   rand_alphaN_max,rand_epsilonF_max,rand_cF_max,rand_beta_max,rand_sigma_max,rand_remin2_max,rand_reminF_max,rand_rhoval_max,rand_mort2val_max,rand_remPOM_max,rand_vel1_max,rand_vel2_max,rand_kc_max];

theserand=(firstRandNr:(firstRandNr+nrRandIter-1))';

for i=1:length(min_params)-1
    randParam(:,i)=(max_params(i)-min_params(i)).*lathcube(theserand,i)+min_params(i);
end

% for i=1:length(min_params)-1
% randParam(:,i)=(max_params(i)-min_params(i)).*rand(nrRandIter,1)+min_params(i);
% end

a=10^(-4):(10^(-2)-10^(-4))/(nrRandIter-1):10^(-2);
mHTL=10.*a;
for i=length(min_params)
    randParam(:,i)=(max(mHTL)-min(mHTL)).*lathcube(theserand,i)+min(mHTL);
end
...
resultdir=fullfile(pwd,'..','..','results','wide','final_both');
...
for itnr=1:nrRandIter
    ...
    for restorenr=1
        ...
        setHTL(randParam(itnr,mortHTL_rand_idx), mHTL(itnr), false, false);
        for p_point=1:length(lon)
