...
nrRandSteps=1;
nrRandIter=24*2*nrRandSteps+1; % 500;
...
b=repmat(1:24,[1 nrRandSteps]);
ii=repmat(1:nrRandSteps,[24 1]);
ii=ii(:);
% The one with all random
randParam_orig=zeros(1,24);
for i=1:length(min_params)
    randParam_orig(1,i)=(max_params(i)-min_params(i)).*lathcube(thisrand,i)+min_params(i);
end
% random parameters for the ones where only 1 is varied:
lathcube_one=lhsdesign(1,24);
randParamRand_one=zeros(1,24);
for i=1:length(min_params)
    randParamRand_one(1,i)=(max_params(i)-min_params(i)).*lathcube_one(1,i)+min_params(i);
end
randParam1=repmat(randParam_orig,[24,1]);
for i=1:length(min_params)
    randParam1(i,i)=randParamRand_one(1,i);

end

% random parameters for the ones where only 1 is varied:
lathcube_2=lhsdesign(24,24);
randParamRand_all=zeros(24,24);
for j=1:length(min_params)
    for i=1:length(min_params)
        randParamRand_all(j,i)=(max_params(i)-min_params(i)).*lathcube_2(j,i)+min_params(i);
    end
end
randParam2=randParamRand_all;

% the 24 similations where all but one parameter are varied
for i=1:24
    randParam2(i,i)=randParam_orig(1,i);
    
end
% combine all simulations into matrix
randParam=[randParam_orig;randParam1;randParam2];
...
resultdir=fullfile(pwd,'..','..','results','wide','final_Sobols2');
...
for itnr=1:nrRandIter
    ...
    for restorenr=1
        ...
        for p_point=2 %1:length(lon)
            ...
            %
            outname=['p_point_',num2str(p_point),'_restore_',strout(restorenr),'_thisrand',num2str(thisrand),'_nrRanditer_',num2str(itnr),'_',time,'_rand',num2str(arandnr),'.mat'];
